...
| 3 | **Active — skill shipped; broader trial open** | [Collection coordination skill](../skills/collection-coordination/SKILL.md), [coordination recipe](feature-coordination-recipe.md), merged [PR #3](https://github.com/rore/rore-collection/pull/3), and Agent Workflow's [open transition-guidance PR #38](https://github.com/rore/agent-workflow/pull/38). | `@call-dev` coordinates the remaining trial. | Collect real participant and handoff evidence from existing work; add remaining supporting refs when published. Keep untested steps explicit and avoid duplicate dispatch. | Participant board badges and the unmerged Agent Workflow guidance are not prerequisites. Broader all-tool validation remains in 2. |
