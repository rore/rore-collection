...
| 3 | **Active — bounded guidance; skill trial paused** | Clarify shared-roadmap coordination in the existing [coordination recipe](feature-coordination-recipe.md); skill packaging and full trial remain proposed. | `@call-dev` owns this guidance slice; skill implementation is unassigned. | Review the eight-case matrix and live trial scenario, then link the owning tools' exact supporting feature refs when returned. Start skill packaging only after communication and combined instruction behavior are verified. | 1, 2 for skill packaging; none for guidance. |
