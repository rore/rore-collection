...
| 3 | **Active — skill shipped; broader trial open** | [Collection coordination skill](skills/collection-coordination/SKILL.md), [coordination recipe](feature-coordination-recipe.md), and merged [PR #3](https://github.com/rore/rore-collection/pull/3). | `@call-dev` coordinates the remaining trial. | Collect real participant and handoff evidence from existing work; add exact supporting refs when tool owners respond. Keep untested steps explicit and avoid duplicate dispatch. | Participant board badges are not a prerequisite. Broader all-tool validation remains in 2. |
