...
| 3 | **Active — initial skill review; broader trial open** | [Collection coordination skill](skills/collection-coordination/SKILL.md), [coordination recipe](feature-coordination-recipe.md), and [draft PR #3](https://github.com/rore/rore-collection/pull/3). | `@call-dev` owns the accepted skill assignment and trial. | Review PR #3 and its controlled failure cases, then merge the initial skill if checks pass. Exact supporting refs are requested from tool owners; no new worker dispatch is needed. Continue real participant/handoff evidence later and keep untested steps explicit. | Participant board badges are not a prerequisite. Broader all-tool validation remains in 2. |
